...
export class ZoteroWriter {
  ...
  /** Verify the credentials before anything is written. */
  async verify() {
    const res = await fetch(`${API}/users/${encodeURIComponent(this.userid)}/items?limit=1`, {
      headers: this.headers,
      signal: AbortSignal.timeout(20000),
    });
    if (res.status === 403) {
      throw new Error('Zotero rejected the API key (403). Check the key has write access '
        + 'to your library, and that the userID is the numeric one from '
        + 'zotero.org/settings/keys.');
    }
    if (res.status === 404) {
      throw new Error(`Zotero has no user ${this.userid} (404). The userID is the number `
        + 'shown on zotero.org/settings/keys, not your username.');
    }
  ...
  /** Returns a Map of refNumber -> zoteroItemKey. */
  async create(itemsByN, onProgress = null) {
    ...
    for (let start = 0; start < pending.length; start += 50) {
      ...
      if (!res.ok) {
        throw new Error(`Zotero rejected the write: HTTP ${res.status} `
      ...
      for (const [idx, info] of Object.entries(body.failed || {})) {
        this.log('warn', `[${chunk[Number(idx)][0]}] Zotero create failed: ${JSON.stringify(info)}`);
      }
